...
class UpdateEmployerCoordinates extends Command
{
    protected $signature = 'employer:update-coordinates';
    protected $description = 'Update employer coordinates based on state';
    ...
    protected const DEFAULT_US_COORDINATES = ['lat' => 39.8283, 'lng' => -98.5795]; // Geographic center of contiguous US (Lebanon, Kansas)

    public function handle()
    {
        // First, let's check total records
        $total = Employer::count();
        $this->info("Total employers: {$total}");

        // Check records with null coordinates
        $nullCoords = Employer::whereNull('latitude')
            ->whereNull('longitude')
            ->count();
        $this->info("Employers with null coordinates: {$nullCoords}");

        // Check records with states
        $withState = Employer::whereNotNull('state')->count();
        $this->info("Employers with states: {$withState}");

        // Check US records
        $usRecords = Employer::where('country', 'United States')->count();
        $this->info("US employers: {$usRecords}");

        // Update country to 'United States' where it is null
        $updatedCountryCount = Employer::whereNull('country')
            ->update(['country' => 'United States']);
        $this->info("Updated country for {$updatedCountryCount} employers.");

        // Now the full query - only check for US employers
        $employers = Employer::where('country', 'United States')
            ->get();

        $this->info("Found {$employers->count()} employers to process");

        $count = 0;
        foreach ($employers as $employer) {
            if ($employer->state === null) {
                // Use default US coordinates for null states
                try {
                    $result = $employer->update([
                        'latitude' => self::DEFAULT_US_COORDINATES['lat'],
                        'longitude' => self::DEFAULT_US_COORDINATES['lng'],
                    ]);

                    if ($result) {
                        $count++;
                        $this->info("Successfully updated employer ID: {$employer->id} with default US coordinates");
                    }
                } catch (\Exception $e) {
                    $this->error("Error updating employer ID: {$employer->id}: " . $e->getMessage());
                }
                continue;
            }

            $state = ucwords(strtolower(trim($employer->state)));
            $this->info("Processing employer ID: {$employer->id}, State: {$state}");
            ...
            if (isset(self::STATE_COORDINATES[$state])) {
                try {
                    ...
                    if ($result) {
                        $count++;
                        $this->info("Successfully updated employer ID: {$employer->id}");
                    } else {
                        $this->error("Failed to update employer ID: {$employer->id}");
                    }
                } catch (\Exception $e) {
                    $this->error("Error updating employer ID: {$employer->id}: " . $e->getMessage());
                }
        ...
        $this->info("Updated coordinates for {$count} employers.");
    }
}