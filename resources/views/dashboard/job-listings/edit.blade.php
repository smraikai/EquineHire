...
@php
    $metaTitle = "Edit {$jobListing->title} | EquineHire";
    $pageTitle = "Edit Job Listing: {$jobListing->title}";
@endphp
...
@section('content')
    <div class="container py-12 mx-auto sm:py-24">
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('dashboard.job-listings.update', $jobListing) }}" method="POST"
                class="p-6 bg-white rounded-lg shadow-sm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex space-x-4">
                        <div class="flex items-center">
                            <input type="radio" id="remote_position_yes" name="remote_position" value="1"
                                ...
                                {{ old('remote_position', $jobListing->remote_position) == '1' ? 'checked' : '' }}
                                required>
                        ...
                        <div class="flex items-center">
                            <input type="radio" id="remote_position_no" name="remote_position" value="0"
                                class="text-blue-600 border-gray-300 focus:ring-blue-500"
                                {{ old('remote_position', $jobListing->remote_position) == '0' ? 'checked' : '' }}
                                required>
                ...
                <div class="mb-6">
                    ...
                    <select name="job_type" id="job_type"
                        ...
                        <option value="full-time"
                            {{ old('job_type', $jobListing->job_type) == 'full-time' ? 'selected' : '' }}>Full Time
                        </option>
                        <option value="part-time"
                            {{ old('job_type', $jobListing->job_type) == 'part-time' ? 'selected' : '' }}>Part Time
                        </option>
                        <option value="contract"
                            {{ old('job_type', $jobListing->job_type) == 'contract' ? 'selected' : '' }}>Contract</option>
                        <option value="temporary"
                            {{ old('job_type', $jobListing->job_type) == 'temporary' ? 'selected' : '' }}>Temporary
                        </option>
                    </select>
                    @error('job_type')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                ...
                <div class="mb-6">
                    <label for="experience_required" class="block text-sm font-medium text-gray-700">Experience Required
                ...
                <div class="mb-6">
                    ...
                    <select name="salary_type" id="salary_type"
                        ...
                        <option value="hourly"
                            {{ old('salary_type', $jobListing->salary_type) == 'hourly' ? 'selected' : '' }}>Hourly
                        </option>
                        <option value="annual"
                            {{ old('salary_type', $jobListing->salary_type) == 'annual' ? 'selected' : '' }}>Annual
                        </option>
                    </select>
                    @error('salary_type')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                ...
                <div id="hourly_rate_fields" class="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-2" style="display: none;">
                    <div>
                        <label for="hourly_rate_min" class="block text-sm font-medium text-gray-700">Hourly Rate Min
                            <span class="text-red-500">*</span></label>
                        <div
                    ...
                    <div>
                        <label for="hourly_rate_max" class="block text-sm font-medium text-gray-700">Hourly Rate Max
                            <span class="text-red-500">*</span></label>
                        <div
                            class="flex items-center mt-1 overflow-hidden border border-gray-300 rounded-md focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500">
                            <span class="pl-3 text-gray-500 sm:text-sm">$</span>
                ...
                <div id="annual_salary_fields" class="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-2" style="display: none;">
                    <div>
                        <label for="salary_range_min" class="block text-sm font-medium text-gray-700">Salary Range Min
                            <span class="text-red-500">*</span></label>
                        <div
                            class="flex items-center mt-1 overflow-hidden border border-gray-300 rounded-md focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500">
                            <span class="pl-3 text-gray-500 sm:text-sm">$</span>
                    ...
                    <div>
                        <label for="salary_range_max" class="block text-sm font-medium text-gray-700">Salary Range Max
                            <span class="text-red-500">*</span></label>
                        <div
                            class="flex items-center mt-1 overflow-hidden border border-gray-300 rounded-md focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500">
                            <span class="pl-3 text-gray-500 sm:text-sm">$</span>
                ...
                <div id="application_link_container" class="mb-6" style="display: none;">
                    <label for="application_link" class="block text-sm font-medium text-gray-700">Application Link
                        <span class="text-red-500">*</span></label>
